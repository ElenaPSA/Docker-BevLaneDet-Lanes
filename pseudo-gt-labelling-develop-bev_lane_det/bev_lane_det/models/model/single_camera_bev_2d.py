...
class InstanceEmbedding_offset_y(nn.Module):
    def __init__(self, ci, co=1):
        super(InstanceEmbedding_offset_y, self).__init__()
...
class LaneHeadResidual_Instance_with_offset(nn.Module):
    def __init__(self, output_size, input_channel=256):
        super(LaneHeadResidual_Instance_with_offset, self).__init__()
        ...
        self.head = InstanceEmbedding_offset_y(64, 2)
...
class BEV_LaneDet(nn.Module):  # BEV-LaneDet
    def __init__(self, bev_shape, output_2d_shape, train=True):
        ...
        self.lane_head = LaneHeadResidual_Instance_with_offset(bev_shape, input_channel=512)
        self.is_train = train
        if self.is_train:
