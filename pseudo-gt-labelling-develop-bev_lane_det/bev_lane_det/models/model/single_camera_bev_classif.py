...
class InstanceEmbedding_offset_y_z(nn.Module):
    def __init__(self, ci, co=1):
        ...
        self.m_category_new = nn.Sequential(
            # nn.Dropout2d(0.2),
            nn.Conv2d(ci, 128, 3, 1, 1, bias=False),
            nn.BatchNorm2d(128),
            nn.ReLU(),
            ...
            nn.Conv2d(64, N_CLASS, 3, 1, 1, bias=True)
        )
        ...
        naive_init_module(self.m_category_new)
        naive_init_module(self.neck_new)

    def forward(self, x):
        feat = self.neck_new(x)
        return self.ms_new(feat), self.me_new(feat), self.m_offset_new(feat), self.m_z(feat), self.m_category_new(feat)
