...
def load_checkpoint(checkpoint, net, optimizer=None, map_loc="cuda"):
    ...
    # for k in sd["model_state"].copy():
    #     if "category" in k:
    #         sd["model_state"].pop(k)
...
def load_model(model, model_state_file):
    ...
    pretrained_dict = {
        k[6:]: v for k, v in pretrained_dict1.items() if k[6:] in model_dict.keys()
    }
    if len(pretrained_dict) == 0:
        pretrained_dict = {
            k: v for k, v in pretrained_dict1.items() if k in model_dict.keys()
        }
    count = 0
