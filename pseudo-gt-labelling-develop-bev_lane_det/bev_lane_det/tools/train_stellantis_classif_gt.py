...
def train_epoch(model, dataset, optimizer, configs, epoch, writer):
    ...
    for idx, (
        ...
        loss_back_total = (
            loss_back_bev + 0.5 * loss_back_2d + loss_offset + loss_z + loss_classif
        )
        """ caclute loss """
        ...
        if idx % 50 == 0:
            loss_iter = {
                ...
                "z loss": loss_z.item(),
                "classif loss": loss_classif.item(),
            }
        ...
        if idx % 300 == 0:
            ...
            loss_iter = {
                ...
                "z_loss": loss_z.item(),
                "classif_loss": loss_classif.item(),
    ...
    loss_iter = {
        ...
        "epoch/classif_loss": loss_classif.item(),
...
def validate(model, dataset, configs, epoch):
    ...
    for idx, (
        ...
        image_gt_segment = image_gt_segment.cuda()
        image_gt_instance = image_gt_instance.cuda()
        prediction = model(input_data, train=False)
        target = gt_seg_data.detach().cpu().numpy().ravel()
        pred = torch.sigmoid(prediction).detach().cpu().numpy().ravel()
        f1_bev_seg = f1_score(
            (target > 0.5).astype(np.int64),
...
def worker_function(config_file, gpu_id, checkpoint_path=None):
    ...
    """ dataset """
    if configs.balance:
        dataset, sampler = configs.train_dataset()
    ...
    for epoch in range(configs.start_epoch, configs.epochs):
        ...
        if configs.with_validation:
            ...
            writer.add_scalar("epoch/F1_bev_seg_val", f1_val, epoch)
            if f1_val >= best_val:
                best_val = f1_val
...
if __name__ == "__main__":
    ...
    worker_function(
        args.config,
        gpu_id=[0, 1, 2, 3],
        # checkpoint_path="/data/gvincent/pseudo-gt-labelling/bev_lane_det/checkpoints/stl_lane_postprocessed/latest.pth",
    )
