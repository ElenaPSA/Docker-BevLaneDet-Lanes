...
class BEV_LaneDetRes50(nn.Module):  # BEV-LaneDet
    def __init__(self, bev_shape, output_2d_shape, train=True):
        super(BEV_LaneDetRes50, self).__init__()
        ...
        self.s32transformer = FCTransform_((2048, 18, 32), (512, 25, 5))
        self.s64transformer = FCTransform_((4096, 9, 16), (512, 25, 5)) 

        self.lane_head = LaneHeadResidual_Instance_with_offset_z(bev_shape, input_channel=1024)
        self.is_train = train
    ...
    def forward(self, img):
        ...
        # else:
        #     return self.lane_head(bev)
class BEV_LaneDetDla34(nn.Module):  # BEV-LaneDet
    def __init__(self, bev_shape, output_2d_shape, train=True):
        super(BEV_LaneDetDla34, self).__init__()

       
        self.bb = nn.Sequential(*list(timm.create_model('dla34', features_only=True,pretrained=True).children()))
        print(self.bb)
        self.down = naive_init_module(
            Residual(
                module=nn.Sequential(
