...
def train_epoch(model, dataset, optimizer, configs, epoch, writer):
    ...
    input_data, gt_seg_data, gt_emb_data, offset_y_data, z_data, image_gt_segment, image_gt_instance) in enumerate(
        ...
        optimizer.step()
        if idx % 50 == 0:
            loss_iter = {"total loss":loss_back_total.item(), "BEV Loss": loss_back_bev.item(), 'offset loss': loss_offset.item(), 'z loss': loss_z.item()}
        ...
        if idx % 300 == 0:
            ...
            loss_iter = {"total_loss":loss_back_total.item(), "BEV_Loss": loss_back_bev.item(), 'offset_loss': loss_offset.item(), 'z_loss': loss_z.item(),
    ...
    pred = torch.sigmoid(prediction).detach().cpu().numpy().ravel()
    f1_bev_seg = f1_score((target > 0.5).astype(np.int64), (pred > 0.5).astype(np.int64), zero_division=1)
    loss_iter = {"epoch/total_loss":loss_back_total.item(), "epoch/BEV_Loss": loss_back_bev.item(), 'epoch/offset_loss': loss_offset.item(), 'epoch/z_loss': loss_z.item(),
