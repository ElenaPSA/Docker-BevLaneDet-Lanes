...
class Combine_Model_and_Loss(torch.nn.Module):
    ...
    def forward(self, inputs, gt_seg=None, gt_instance=None, gt_offset_y=None, image_gt_segment=None,
        ...
        if train:
            ...
            return pred, loss_total, loss_total_2d, loss_offset
        else:
            return pred
...
def train_epoch(model, dataset, optimizer, configs, epoch, writer):
    ...
    input_data, gt_seg_data, gt_emb_data, offset_y_data, z_data, image_gt_segment, image_gt_instance) in enumerate(
        ...
        prediction, loss_total_bev, loss_total_2d, loss_offset = model(input_data,
                                                                        gt_seg_data,
                                                                        gt_emb_data,
                                                                        offset_y_data,
                                                                        image_gt_segment,
        ...
        loss_back_total = loss_back_bev + 0.5 * loss_back_2d + loss_offset
        ''' caclute loss '''
        ...
        optimizer.step()
        if idx % 50 == 0:
            loss_iter = {"total loss":loss_back_total.item(), "BEV Loss": loss_back_bev.item(), 'offset loss': loss_offset.item()}
        ...
        if idx % 300 == 0:
            ...
            loss_iter = {"total_loss":loss_back_total.item(), "BEV_Loss": loss_back_bev.item(), 'offset_loss': loss_offset.item(),
    ...
    pred = torch.sigmoid(prediction).detach().cpu().numpy().ravel()
    f1_bev_seg = f1_score((target > 0.5).astype(np.int64), (pred > 0.5).astype(np.int64), zero_division=1)
    loss_iter = {"epoch/total_loss":loss_back_total.item(), "epoch/BEV_Loss": loss_back_bev.item(), 'epoch/offset_loss': loss_offset.item(),
...
if __name__ == '__main__':
    ...
    worker_function('./openlane_config.py', gpu_id=[0,1,2,3])#, checkpoint_path='/data/gvincent/bev_lane_det/checkpoints/openlane/ep008.pth')
