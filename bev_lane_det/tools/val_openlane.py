...
from utilities.display_utils import Line_projector
from utilities import DATA_DIR

import pdb

run_device = device('cuda' if torch.cuda.is_available() else 'cpu')
# model_path = '/dataset/model/20220814/ep008.pth' #model path of verification
model_path = '/data/gvincent/bev_lane_det/checkpoints/openlane/ep008.pth' #model path of verification
model_path = '/data/gvincent/bev_lane_det/checkpoints/openlane/ep010.pth' #model path of verification
model_path = '/data/gvincent/bev_lane_det/checkpoints/openlane/ep020.pth' #model path of verification
model_path = '/data/gvincent/bev_lane_det/checkpoints/openlane/ep030.pth' #model path of verification
model_path = '/data/gvincent/bev_lane_det/checkpoints/openlane/ep039.pth' #model path of verification
model_path = '/data/gvincent/bev_lane_det/checkpoints/openlane/ep047.pth' #model path of verification
...
tmp_save_path =  str(DATA_DIR) + '/tmp/tmp_openlane' #tmp path for save intermediate result
...
def val(model_path):
    ...
    # Display lines in BEV and in camera frame
    visualizer = Line_projector("stellantis_val",
                                tmp_save_path,
                                configs.x_range,
                                configs.y_range,
                                configs.meter_per_pixel)

    res_list = os.listdir(res_save_path)
    img_list = configs.val_dataset().cnt_list
    img_cam_outdir = os.path.join(res_save_path, "img_cam")
    img_bev_outdir = os.path.join(res_save_path, "img_bev")
    if not os.path.isdir(img_cam_outdir):
        os.makedirs(img_cam_outdir)
    if not os.path.isdir(img_bev_outdir):
        os.makedirs(img_bev_outdir)
        
    for item in tqdm(postprocess.valid_data):
        
        files = item.split('.')[0].split('__')
        gt_path = os.path.join(gt_paths, files[0], files[1] + '.json')
        with open(gt_path, 'r') as f:
            gt = json.load(f)
        result_path = os.path.join(res_save_path, files[1] + ".json")
        if os.path.isfile(result_path):
            with open(result_path, 'r') as f:
                res = json.load(f)
        img_file = os.path.join(image_paths, files[0], files[1] + ".jpg")
        cam_disp = visualizer.draw_on_img_category(img_file, res[0], res[1],
                                                    np.array(gt['extrinsic']),
                                                    np.array(gt['intrinsic']))

        out_file = files[1] + "_inference.png"
        cv2.imwrite(os.path.join(img_cam_outdir, out_file), cam_disp)

        # imp_disp = visualizer.draw_on_bev_category(res[0], res[1])
...
if __name__ == '__main__':
    model_path = os.path.join(DATA_DIR,'models/run1_ep008.pth') #model path of verification
    val(model_path)
