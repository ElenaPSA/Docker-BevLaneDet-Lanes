...
from utilities.util_val.MinCostFlow import SolveMinCostFlow
from utilities.util_val.utils import *
...
class LaneEval(object):
    ...
    def bench(self, pred_lanes, gt_lanes):
        ...
        gt_lanes = [
            prune_3d_lane_by_range(np.array(lane), 3 * self.x_min, 3 * self.x_max)
            for lane in gt_lanes
        ]
    ...
    def bench_all(self, pred_lanes, gt_lanes):
        (
            ...
            z_error_close,
            z_error_far,
        ) = self.bench(pred_lanes, gt_lanes)
    ...
    def show(self):
        ...
        self.laneline_x_error_close = np.array(self.laneline_x_error_close)[
            ~np.isnan(self.laneline_x_error_close)
        ]
        self.laneline_x_error_far = np.array(self.laneline_x_error_far)[
            ~np.isnan(self.laneline_x_error_far)
        ]
        self.laneline_z_error_close = np.array(self.laneline_z_error_close)[
            ~np.isnan(self.laneline_z_error_close)
        ]
        self.laneline_z_error_far = np.array(self.laneline_z_error_far)[
            ~np.isnan(self.laneline_z_error_far)
        ]
        dict_res = {
