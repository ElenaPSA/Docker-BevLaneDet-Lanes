...
class OpenLane_dataset_with_offset_val(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        # #### DEBUG ####
        # pdb.set_trace()
        # image_disp = image.numpy()
        # image_disp = image_disp.transpose((1,2,0))
        # cv2.imwrite("openlane_img_transform.png", image_disp)
        # ###############
...
class OpenLane_dataset_with_offset_classif(Dataset):
    def __init__(self, image_paths, 
                   gt_paths,
                   x_range,
                   y_range, 
    ...
    def get_y_offset_and_z_and_classif(self, res_d, category_d):
        '''
        :param res_d: res_d
        :param instance_seg:
        :return:
        ...
        for idx in res_d:
            ...
            res_lane_points_set[idx] = np.array([base_points, y_points]).astype(
                np.int)
            res_lane_category_bin[idx] = category_d[idx]
        ...
        for idx in res_lane_points_bin:
            lane_bin = res_lane_points_bin[idx].T
            category = res_lane_category_bin[idx]
            for point in lane_bin:
                row, col = point[0], point[1]
                if not (0 < row < self.ipm_h and 0 < col < self.ipm_w):  #
                    continue
    ...
    def get_seg_offset(self, idx, smooth=False):
        ...
        for idx in range(len(lanes)):
            ...
            category = int(lane1['category'])
            category = category if category<13 else category-7
            category = category if category<15 else 0
            category_d[idx + 1] = category

        ipm_gt, offset_y_map, z_map, category_map = self.get_y_offset_and_z_and_classif(res_points_d, category_d)
        ...
        if self.use_virtual_camera:
            ...
            image = cv2.warpPerspective(image, trans_matrix, self.vc_image_shape)
            image_gt = cv2.warpPerspective(image_gt, trans_matrix, self.vc_image_shape)
        return image, image_gt, ipm_gt, offset_y_map, z_map, cam_extrinsics, cam_intrinsic, category_map
    ...
    def __getitem__(self, idx):
        ...
        :return:
        '''
        image, image_gt, ipm_gt, offset_y_map, z_map, cam_extrinsics, cam_intrinsic, category_map = self.get_seg_offset(idx)
        transformed = self.trans_image(image=image)
        image = transformed["image"]
        ''' 2d gt'''
        image_gt = cv2.resize(image_gt, (self.output2d_size[1], self.output2d_size[0]), interpolation=cv2.INTER_NEAREST)
        ...
        ipm_gt_segment = torch.clone(ipm_gt_instance)
        ipm_gt_segment[ipm_gt_segment > 0] = 1
        ipm_gt_category = torch.tensor(category_map)
        return image, ipm_gt_segment.float(), ipm_gt_instance.float(), ipm_gt_offset.float(), ipm_gt_z.float(), ipm_gt_category.long(), image_gt_segment.float(), image_gt_instance.float()

    def __len__(self):
        return len(self.cnt_list)


class OpenLane_dataset_2d(Dataset):
    def __init__(self, image_paths, 
                   gt_paths,
                   data_trans,
                   output_2d_shape,
    ...
    def get_seg_offset(self, idx, smooth=False):
        ...
        image_gt = np.zeros((image_h, image_w), dtype=np.uint8)
        image_category_gt = np.zeros((image_h, image_w), dtype=np.uint8)
        for idx in range(len(lanes)):
            lane1 = lanes[idx]
            lane_camera_w = np.array(lane1['xyz']).T[np.array(lane1['visibility']) == 1.0].T
            lane_camera_w = np.vstack((lane_camera_w, np.ones((1, lane_camera_w.shape[1]))))
            ...
            category = category if category<13 else category-7
            category = category if category<15 else 0
            cv2.polylines(image_category_gt, [uv1[0:2, :].T.astype(np.int)], False, category, self.lane2d_thick)
        ...
        if self.use_virtual_camera:
            ...
            image = cv2.warpPerspective(image, trans_matrix, self.vc_image_shape)
            image_gt = cv2.warpPerspective(image_gt, trans_matrix, self.vc_image_shape)
            image_category_gt = cv2.warpPerspective(image_category_gt, trans_matrix, self.vc_image_shape)
        return image, image_gt, image_category_gt, cam_extrinsics, cam_intrinsic
    ...
    def __getitem__(self, idx):
        ...
        :return:
        '''
        image, image_gt, image_category_gt, cam_extrinsics, cam_intrinsic = self.get_seg_offset(idx)
        transformed = self.trans_image(image=image)
        image = transformed["image"]
        ''' 2d gt'''
        image_gt = cv2.resize(image_gt, (self.output2d_size[1], self.output2d_size[0]), interpolation=cv2.INTER_NEAREST)
        ...
        image_gt_segment = torch.clone(image_gt_instance)
        image_gt_segment[image_gt_segment > 0] = 1
        image_category_gt = cv2.resize(image_category_gt, (self.output2d_size[1], self.output2d_size[0]), interpolation=cv2.INTER_NEAREST)
        image_category_gt = torch.tensor(image_category_gt)

        return image, image_gt_segment.float(), image_gt_instance.float(), image_category_gt.long()
...
if __name__ == "__main__":
    ...
    config_file = os.apth.join(PROJECT_DIR, '/tools/openlane_config.py')
    ...
    for item in dataset:
        break
        continue
    # image, ipm_gt_segment, ipm_gt_instance, ipm_gt_offset, ipm_gt_z, ipm_gt_category, image_gt_segment, image_gt_instance = item
    image, image_gt_segment, image_gt_instance, image_category_gt = item
    # image, _ = item
    import matplotlib.pyplot as plt
    image = image.permute([1,2,0]).numpy()[:,:,::-1]
    plt.imshow(image)
    plt.show()
    # for im in [ipm_gt_segment, ipm_gt_instance, ipm_gt_offset, ipm_gt_z, image_gt_segment, image_gt_instance]:
    #     plt.imshow(im[0])
    #     plt.show()